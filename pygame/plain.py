...
class LoadingObject(pg.sprite.Sprite):
    def __init__(self):
        ...
        image_path = 'animations/Loading2'
        ...
        for img in img_list:
            if img == ".DS_Store":
                ...
                self.image_sprite.append(pg.image.load(os.path.join(image_path, img)))
    ...
    def render(self, display):
        ...
        if cycle_start:
            ...
            self.image = pg.image.load('images/default.png')
            self.image = pg.transform.scale(self.image, (300, 300))
            surface = pg.display.get_surface()
            x, y = size = surface.get_width(), surface.get_height()
...
class LogoObject(pg.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pg.image.load('images/logo2.png')
        self.image = pg.transform.scale(self.image, (450, 150))
...
btn = Button(rect=(150, 450, 105, 25), command=lambda: get_textboxes(fill_entry.final, wash_entry.final), text='OK',
             **btn_settings)
logo = LogoObject()
loading = LoadingObject()

myfont = pg.font.SysFont("arial", 20, True, False)
filllabel = myfont.render("Fill      (Gal): ", 0, (255,255,0))
